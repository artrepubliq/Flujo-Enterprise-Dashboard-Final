...
export class LoginComponent implements OnInit {
  ...
  constructor(private router: Router,private alertService: AlertService, private formBuilder: FormBuilder, private spinnerService: Ng4LoadingSpinnerService, private httpClient: HttpClient,private authService: AuthService,private loginAuthService:LoginAuthService){
    this.loginForm = this.formBuilder.group({
      // 'user_name': ['', Validators.required],
      'user_name': ['', Validators.pattern('^[a-zA-Z \-\']+')],
      'user_password':['',Validators.required],
    });
    if(this.loginAuthService.customLoggedIn){
  ...
  ngOnInit() {
    if(this.loginAuthService.hasPingHandle){
...
//       this.router.navigate(['/login']);
//     }
//this.loginAuthService._setSession;
  }
