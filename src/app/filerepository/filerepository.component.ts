...
export class FilerepositoryComponent implements OnInit {
    ...
    disabled: boolean = true;
	  ...
	  @Input() fileExt: string = "JPG, PDF, PNG, JPEG, CSV, DOCX, DOC";
	  @Input() maxFiles: number = 1;
	  @Input() maxSize: number = 2; // 5MB
    ...
    onSubmit() {
      let fileData = this.FileUploadControl.value;
    ...
    private isValidFileSize(file) {
          ...
          if (size > this.maxSize){
              this.disabled = true;
              this.alertService.warning("Error (File Size): " + file.name + ": exceed file size limit of " + this.maxSize + "MB ( " + size + "MB )");
